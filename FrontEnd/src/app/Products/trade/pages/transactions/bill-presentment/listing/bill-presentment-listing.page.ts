...
import {BillPresentmentViewPage} from "../view/bill-presentment-view.page";

@Component({
  selector: "bill-presentment-listing",
  templateUrl: "./bill-presentment-listing.page.html",
  styleUrls: ["./bill-presentment-listing.page.scss"],
  providers: [TranslatePipe]
})
export class BillPresentmentListingPage implements OnDestroy, BaseTransaction {
  ...
  ionViewDidEnter() {
    this.dataItemList$ = this.listingService.dataItemList$
      .pipe(takeUntil(this.unsub$)).subscribe((val) => {
        if (
          this.listingService.selectedTab.entityName === "TRADEBILLPRESENTMENT"
        ) {
  ...
  parseViewData() {
    ...
    this.dataItemList.forEach((element) => {
      dataItems.push({
        ...
        importerName: element.Buyer,
        tradeBillNO: element["LC Number"],
  ...
  viewTransactionCommonService(item, $event: MouseEvent) {
    if (this.multiSelectionMode) {
      ...
      this.router.navigate(["/menu/export-trade/view/tradebillpresentment", item.id]);
    }
  ...
  view(item, $event: MouseEvent): void {
    if (this.multiSelectionMode) {
      ...
      this.transactionService.viewInModal(item.id, BillPresentmentViewPage);
      // this.router.navigate(["menu", "export-trade", "view", "tradebillpresentment", item.id]);
    }
  }
