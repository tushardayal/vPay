...
import {BankGuaranteeAmendViewPage} from "../view/bank-guarantee-amend-view.page";

@Component({
  selector: "bank-guarantee-amend-listing",
  templateUrl: "./bank-guarantee-amend-listing.page.html",
  styleUrls: ["./bank-guarantee-amend-listing.page.scss"],
  providers: [TranslatePipe]
})
export class BankGuaranteeAmendListingPage implements OnDestroy, BaseTransaction {
  ...
  ionViewDidEnter() {
    ...
    this.dataItemList$ = this.listingService.dataItemList$
      .pipe(takeUntil(this.unsub$)).subscribe((val) => {
        if (
          this.listingService.selectedTab.entityName === "BANKGUARANTEEAMEND"
        ) {
  ...
  viewTransactionCommonService(item, $event: MouseEvent) {
    if (this.multiSelectionMode) {
      ...
      this.router.navigate(["/menu/export-trade/view/bankguaranteeamend", item.id]);
    }
  ...
  view(item, $event: MouseEvent): void {
    if (this.multiSelectionMode) {
      ...
      this.transactionService.viewInModal(item.id, BankGuaranteeAmendViewPage);
      // this.router.navigate(["menu", "export-trade", "view", "bankguaranteeamend", item.id]);
    }
  }
  ...
  showHelpPage() {
    const labelArray = [{
      ...
      name: 'bgNo',
      text: this.translate.transform("lbl_bg_no")
    }, {
      name: 'applicantName',
