...
@Component({
  ...
  styleUrls:["./alpha-scroll.scss"],
  templateUrl:"./alpha-scroll.html"
})
export class AlphaScroll implements OnInit, OnChanges, OnDestroy,AfterViewInit {
  @ViewChild("wrapper",{static:false}) mainWrapper: ElementRef;
  ...
  constructor(private elementRef: ElementRef, private orderBy: OrderBy) {
    console.log(elementRef,orderBy)
    ...
    console.log(this.list,this.mainWrapper,this.sidebar)
  }
  ngOnInit() {
  }
  ngAfterViewInit(){
    console.log(this.elementRef.nativeElement.offsetHeight);
    if(this.list.nativeElement ==undefined){
      this.list.nativeElement=this.list["el"];
    }
    console.log(this.list.nativeElement.offsetHeight);
    console.log(this.sidebarTouch);
    setTimeout(() => {
  ...
  ngOnChanges() {
    ...
    let sortedListData: Array<any> = this.orderBy.transform(
      _.orderBy(this.listData, x => x.initial),
      [this.key]
    );
  ...
  calculateDimensionsForSidebar() {
    return {
      height: this.mainWrapper?this.mainWrapper.nativeElement.clientHeight + "px":"0px"
    };
  }
  private setTouchHandlers() {
    let sidebarEle: HTMLElement = this.elementRef.nativeElement.querySelector(
      ".ion-alpha-sidebar"
    );
    ...
    setTimeout(() => {
      ...
      let chooseEle = evt => {
        ...
        if (closestEle && ["LI", "A"].indexOf(closestEle.tagName) > -1) {
          ...
          let letterDivider: any = this.elementRef.nativeElement.querySelector(
      ...
      this.sidebarTouch = new AlloyTouch({
        ...
        touchStart: () => {
          ...
          this.letterIndicatorEle.style.top =
            (window.innerHeight - this.indicatorHeight) / 2 + "px";
          this.letterIndicatorEle.style.left =
            (window.innerWidth - this.indicatorWidth) / 2 + "px";
        },
        touchMove: evt => {
          chooseEle(evt);
        },
  ...
  private unwindGroup(groupItems: any): Array<any> {
    ...
    for (let letter in groupItems) {
      result = result.concat(
        [{ isDivider: true, letter: letter }].concat(groupItems[letter])
      );
    }
    return result;
  }
