...
export class BillPaymentTradeViewPage implements OnInit, OnDestroy {
  id: string;
  dataObj: any = {};
  billPaymentDetails;
  unsub$: Subscription;
  ...
  ngOnInit() {
    ...
    this.unsub$ = this.listingSrv.view(this.id, extraData).subscribe(data => {
      ...
      this.billPaymentDetails = this.dataObj;
    });
  }
  donloadFile() {
    // if (this.billPaymentDetails.supportingDocSysfilename && this.billPaymentDetails.supportingDocFilename) {
    //     this.transactionsSrv.downloadFile(this.singlePaymentSrv.CONSTANTS.DOWNLOAD_FILE,
    //       this.billPaymentDetails.supportingDocFilename, this.billPaymentDetails.supportingDocSysfilename);
    //   }
  }
  ngOnDestroy() {
